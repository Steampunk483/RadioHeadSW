...
// Receiver function without stop-and-wait protocol
...
// Sender function without stop-and-wait protocol
...
bool RH_ASK::send (const uint8_t* data, uint8_t len)
{
	...
	if (len > RH_ASK_MAX_MESSAGE_LEN) {
		...
		Serial.println("Message exceeds max message length!");
		return false;
	}
...
// A trimmed-down sender for sending acknowledgements
bool RH_ASK::send_ack (uint8_t ID)
{
	uint8_t i;
	uint16_t index = 0;
	uint16_t crc = 0xffff;
	uint8_t *p = _txBuf + RH_ASK_PREAMBLE_LEN; // start of the message area
	uint8_t data = 0xFF; // ACK symbol (NOTE: not ASCII compliant)
	uint8_t len = 1;
	uint8_t count = len + 3 + RH_ASK_HEADER_LEN; // Added byte count and FCS and headers to get total number of bytes
	...
	crc = RHcrc_ccitt_update (crc, ID);
	p[index++] = symbols[ID >> 4];
	p[index++] = symbols[ID & 0xf];

	// Encode the message into 6 bit symbols. Each byte is converted into
	// 2 6-bit symbols, high nybble first, low nybble second
	crc = RHcrc_ccitt_update (crc, data);
	p[index++] = symbols[(int)data >> 8];
	p[index++] = symbols[(int)data & 0xff];
...
// Receiver function with stop-and-wait protocol
// Note that this version returns a uint8_t, not a Boolean
uint8_t RH_INTERRUPT_ATTR RH_ASK::recvSW (uint8_t* buf, uint8_t* len, bool send_ack_v)
{
	uint8_t message_ID = 0; // ID of received message
	if (!available())
		return 0;
	...
	if (buf && len)
	{
		...
		if (*len > message_len)
			*len = message_len;
		memcpy (message_ID, _rxBuf + RH_ASK_HEADER_LEN + 1, 1);	// Copies the message ID (1 byte)
		memcpy (buf, _rxBuf + RH_ASK_HEADER_LEN + 2, *len); // Copies the message
															// Message starts 1 byte later
															// due to presence of message ID
	}
	if (send_ack_v && (strpbrk(buf, ack) != buf)) {
		send_ack(message_ID);
	}
	_rxBufValid = false; // Got the most recent message, delete it
...
// Sender function with stop-and-wait protocol
// Caution: this may block
bool RH_ASK::sendSW (const uint8_t* data, uint8_t len, uint8_t timeout_delay)
{
	uint8_t i;
	uint16_t index = 0;
	uint16_t crc = 0xffff;
	...
	uint8_t ID = (uint8_t)(random()%256); // Generate a random 8-bit message ID
	crc = RHcrc_ccitt_update (crc, ID);
	p[index++] = symbols[ID >> 8];
	p[index++] = symbols[ID & 0xff];
	...
	// Now that the message has been sent, we need to wait for a response
	static uint8_t max_attempts = 10; // max number of retransmissions before giving up
	uint8_t attempts = 0; // current attempt
	while (attempts < max_attempts) {
		uint8_t rx_buf[RH_ASK_MAX_MESSAGE_LEN] = {};
    	uint8_t rx_buflen = sizeof(rx_buf);
		while (timer.read() < (6*timeout_delay)) { // Ignore the scaling factor
												   // I just put that there so I could more easily
												   // watch the output
			if ((recvSW (rx_buf, &rx_buflen, false) == ID) && (*rx_buf == 0xFF)) {
		...
		attempts = attempts + 1;
		Serial.println("No acknowledgement received! Retransmitting...");
		timer.stop();
		waitPacketSent();
		setModeTx();
		timer.start();
	...
	timer.stop();
	Serial.println("No acknowledgement received after maximum number of retries!");
	return false;
}
