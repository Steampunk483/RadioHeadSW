...
public:
    ...
    /// Add a receiver version for stop-and-wait protocol
    RH_INTERRUPT_ATTR virtual uint8_t recvSW (uint8_t* buf, uint8_t* len, bool send_ack_v);

	/// Add a transmitter version for stop-and-wait protocol
	/// \param[in] timeout_delay The delay before the transmission times out (in milliseconds)
